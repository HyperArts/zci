...
/* include everything in the includes directory top level only */
foreach ( glob(dirname(__FILE__) . "/lib/includes/*.php") as $filename)
{
	include $filename;
}
...
function themeprefix_add_css_attr( $attributes ) {
	...
	$attributes['class'] .= ' small-13 bold upper';
...
add_filter( 'wp_nav_menu_args', 'ha_footer_primary_menu_args' );
/**
 * Add class to menu without theme location.
 *
 * @since 2.2.3
 *
 ...
 */
function ha_footer_primary_menu_args( $args ) {

	if ( $args['menu'] instanceof WP_Term && 'Footer' === $args['menu']->name ) {
		$args['container_class'] = 'menu-primary-container small-14 semi';
	}
...
//* Add custom body class based on user cookie
...
add_action( 'genesis_header', 'ha_add_breadcrumbs', 1 );
//* Adds breadcrumbs to Landing page
