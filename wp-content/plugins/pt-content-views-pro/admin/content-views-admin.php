...
class PT_Content_Views_Pro_Admin extends PT_Content_Views_Admin {
	...
	public function enqueue_admin_scripts() {
		...
		if ( strpos( $screen->id, PT_CV_DOMAIN ) !== false ) {
			...
			PT_CV_Asset::localize_script(
				...
				'custom_field'			 => array(
					'type_operator' => PT_CV_Values_Pro::ctf_filter_operator_each()
				),
				'enable_toggle_settings' => apply_filters( PT_CV_PREFIX_ . 'enable_toggle_settings', true ),
				'hybrid_data'			 => apply_filters( PT_CV_PREFIX_ . 'localize_hybrid', [] ),
		...
		$settings[] = array(
			...
			'params'		 => array(
				array(
					'type'		 => 'checkbox',
					'name'		 => 'post-border',
					'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Show border between posts', 'content-views-pro' ) ),
					'std'		 => '',
				),
			),
		);
	...
	public function filter_field_display( $args, $prefix ) {
		...
		if ( function_exists( 'wc_review_ratings_enabled' ) && wc_review_ratings_enabled() ) {
			$args[] = array(
				...
				'params'		 => array(
					array(
						'type'		 => 'checkbox',
						'name'		 => $prefix . 'woorating',
						'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Show Star Ratings', 'content-views-pro' ) ),
						'std'		 => '',
					),
				),
				'dependence'	 => array( 'content-type', 'product' ),
		...
		$args[] = array(
			...
			'params'		 => array(
				array(
					'type'		 => 'checkbox',
					'name'		 => $prefix . 'woohooks',
					'options'	 => PT_CV_Values::yes_no( 'yes', __( "Do WooCommerce 'loop_item' actions (of extensions, other plugins)", 'content-views-pro' ) ),
					'std'		 => '',
				),
			),
			'dependence'	 => array( 'content-type', 'product' ),
							...
							array(
								...
								'params'		 => array(
									array(
										...
										'options'	 => PT_CV_Values_Pro::imgsub_role(),
										'std'		 => '',
									),
								),
							),
							...
							array(
								...
								'params'		 => array(
									array(
										...
										'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Hide the default image (if no image/video/audio found)' ) ),
										'std'		 => '',
									),
								),
							),
	...
	public function filter_settings_other( $args, $prefix ) {
		...
		$social_style = array(
			...
			'params'	 => array(
				array(
					'type'		 => 'checkbox',
					'name'		 => $prefix . 'social-circle',
					'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Use circle buttons', 'content-views-pro' ) ),
					'std'		 => '',
				),
			),
			'dependence' => array( $prefix . 'social-show', 'yes' ),
		...
		array_unshift( $args, $social_style );
			...
			array(
				...
				'params'	 => array(
					array(
						...
						'std'	 => '',
						'desc'	 => sprintf( __( 'Leave empty to load whole page. To load specific part, please enter selector to identify it %s', 'content-views-pro' ), ', for example: <code>#content</code> or <code>#main</code> or <code>.post</code> or <code>.hentry</code> or <code>article</code> or another value depends on the active Theme' ),
					),
				),
				'dependence' => array( $prefix . 'open-in', PT_CV_PREFIX . 'lightbox' ),
			),
	...
	public function filter_excerpt_settings( $args, $prefix ) {
		...
		$args[ 2 ] = array(
			...
			'params' => array(
				array(
					...
					'std'		 => '',
					'desc'		 => __( 'For HTML tags in excerpt', 'content-views-pro' ),
				),
			),
		);
		...
		$args[] = array(
			...
			'params'	 => array(
				array(
					'type'		 => 'html',
					'content'	 => sprintf( '<p style="margin: -8px 0 15px; font-weight: 500;">%s.</p>', __( 'This option might break page layout in some themes, page builders. Please select another option if problem happen', 'content-views-pro' ) ),
				),
			),
			'dependence' => array( 'field-excerpt-allow_html', 'all-tags' ),
		);
		...
		$args[] = array(
			...
			'params'	 => array(
				array(
					...
					'std'	 => '',
					'desc'	 => sprintf( __( 'Separate tags by comma, for example: %s', 'content-views-pro' ), '<code>h1,h2</code>' ),
				),
			),
			...
			'params'		 => array(
				array(
					...
					'params' => array(
						...
						apply_filters( PT_CV_PREFIX_ . 'readmore_extra_settings', [] ),
						array(
							'label'			 => array(
								'text' => '',
							),
							...
							'params'		 => array(
								array(
									'type'		 => 'checkbox',
									'name'		 => $prefix . 'readmore-round',
									'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Show as button with rounded corners', 'content-views-pro' ) ),
									'std'		 => '',
								),
							),
						),
						...
						array(
							...
							'params'		 => array(
								array(
									'type'	 => 'group',
									'params' => array(
										self::_pagination_design_settings( $prefix . 'output-ajax', array( $prefix . 'style', 'regular' ) ),
									),
								),
							),
							'dependence'	 => array( $prefix . 'type', 'ajax' ),
		...
		$args[] = array(
			...
			'params'	 => array(
				array(
					'type'	 => 'group',
					'params' => array(
						self::_pagination_design_settings( $prefix . 'output-normal', array( $prefix . 'type', 'normal' ) ),
					),
				),
			),
			'dependence' => array( 'enable-pagination', 'yes' ),
	...
	static function _pagination_design_settings( $name, $dependence ) {
		return array(
			'label'			 => array(
				'text' => __( 'Button shape', 'content-views-pro' ),
			),
			'extra_setting'	 => array(
				'params' => array(
					'wrap-class' => PT_CV_PREFIX . 'w200',
			...
			'params'		 => array(
				array(
					...
					'options'	 => array(
						...
						'round-button'	 => __( 'Round edge', 'content-views-pro' ),
						'circle-button'	 => __( 'Circle', 'content-views-pro' ) ),
					'name'		 => $name,
					'std'		 => '',
				),
			),
					...
					'params' => array(
										...
										array(
											...
											'params'	 => array(
												array(
													...
													'std'	 => '',
													'desc'	 => PT_CV_Values_Pro::ctf__desc( 'name' ),
												),
											),
						...
						array(
							...
							'params' => array(
								array(
									'type'		 => 'checkbox',
									'name'		 => $prefix . '-enable-linebreak',
									'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Enable line breaks in field value', 'content-views-pro' ) ),
									'std'		 => '',
								),
							),
						),
						...
						array(
							...
							'params'		 => array(
								array(
									...
									'std'	 => get_option( 'date_format' ),
									'desc'	 => PT_CV_Values_Pro::ctf__desc( 'datenew' ),
								),
							),
							'dependence'	 => array( $prefix . '-date-custom-format', 'yes' ),
						),
						array(
							...
							'params'		 => array(
								array(
									...
									'std'	 => '',
									'desc'	 => PT_CV_Values_Pro::ctf__desc( 'dateold' ),
								),
							),
							'dependence'	 => array( $prefix . '-date-custom-format', 'yes' ),
						),
	...
	public function filter_advanced_settings_panel( $args ) {
		...
		$memberpanel = PT_CV_Settings_Pro::filter_membership_settings();

		// Move settings of Date, Custom Fields to 2nd, 3rd position, right after Taxonomy settings
		$args = array_slice( $args, 0, 1, true ) + $date + $custom_field + array_slice( $args, 1, count( $args ) - 1, true ) + $memberpanel;
		...
		$args[] = array(
			...
			'params' => array(
				array(
					...
					'options'	 => PT_CV_Values_Pro::current_author_options(),
					'std'		 => '',
				),
			),
		);
	...
	public function filter_pre_save_view_data( $data ) {
		...
		do_action( PT_CV_PREFIX_ . 'save_view', $data );
	...
	public function action_admin_more_buttons() {
		?>
		<a href="http://www.contentviewspro.com/contact/" target="_blank" class="btn btn-default pull-right pt-cv-report-bug" style="margin-right: 10px; background-color: #ebebeb;">Support</a>
		<a href="https://docs.contentviewspro.com/" target="_blank" class="btn btn-default pull-right pt-cv-report-bug" style="margin-right: 10px; background-color: #ebebeb;">Docs</a>
		<?php
	}
