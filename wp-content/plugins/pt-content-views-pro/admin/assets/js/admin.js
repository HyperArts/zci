	...
	$.PT_CV_Admin_Pro.prototype = {
		...
		_custom_field_common: function ( this_prefix, ctf_table, tpl, ctf_class, ctf_live_filter_enable, toggle_relation, type ) {
			...
			$( '#' + this_prefix + 'add' ).on( 'click', function ( e ) {
				...
				ctf_table.find( 'select.' + this_prefix + 'key' ).last().prev('.select2-container').remove();
		...
		_custom_trigger: function () {
			...
			$( '.pt-wrap' ).on( 'content-type-change', function ( e, content_type ) {
				if ( content_type === 'product' ) {
					...
					$( '[name="' + _prefix + 'field-excerpt-readmore' + '"]' ).prop( 'checked', false ).trigger( 'change' );
				} else {
		...
		_padding_margin: function () {
			$( '<span />', { 'class': 'dashicons dashicons-admin-links', 'style': 'cursor: pointer;' } ).appendTo( '.cv-padding-margin' );
			$( '.cv-padding-margin .dashicons' ).off( 'click' ).on( 'click', function () {
				$( this ).toggleClass( 'dashicons-editor-unlink' );
			} );
			$( '.cv-padding-margin input' ).off( 'keyup change' ).on( 'keyup change', function () {
				var $wrp1 = $( this ).closest( '.cv-padding-margin' ), $icon = $wrp1.find( '.dashicons' );
				if ( !$icon.hasClass( 'dashicons-editor-unlink' ) ) {
					$wrp1.find( 'input' ).val( $( this ).val() );
				}
			} );
		},
		/**
		...
		_toggle_field_settings: function () {
			...
			$( '.' + _prefix + 'field-setting > .control-label' ).on( 'toggle-setting', function ( e, status ) {
				...
				if ( $( this ).parent( '.' + _prefix + 'metafield-setting' ).length > 0 ) {
					...
					$related_settings = $related_settings.add( $( '.' + _prefix + 'metafield-extra1' ) );
				}
		...
		_toggle_hybrid: function () {
			var view_type = '[name="' + _prefix + 'view-type' + '"]';
			var $animation_tab = $( 'a[href*="animation-settings"]' );
			var $animation_styles = $( 'div[class*="font-mask"]', '#' + _prefix + 'style-settings' ).parent();

			var $limit = $( '[name="' + _prefix + 'limit' + '"]' ).closest( '.pt-form-group' );
			var pagination_style = '[name="' + _prefix + 'pagination-style' + '"]';

			var $samesize = $( '[name="' + _prefix + 'field-thumbnail-same-size' + '"]' ).closest( '.pt-form-group' );

			var fn_view_type = function () {
				var this_val = $( view_type + ':checked' ).val();

				// animation
				var show_animation = true;
				var no_animation = PT_CV_ADMIN_PRO.hybrid_data ? PT_CV_ADMIN_PRO.hybrid_data.layouts : null;
				if ( no_animation && no_animation.includes( this_val ) ) {
					show_animation = false;
				}
				$animation_tab.toggle(show_animation);
				$animation_styles.toggle(show_animation);

				$samesize.toggle(show_animation);

				// pagination
				var disable1 = false;
				var no_ppplimit = PT_CV_ADMIN_PRO.hybrid_data ? PT_CV_ADMIN_PRO.hybrid_data.no_ppp_limit : null;
				if ( no_ppplimit && no_ppplimit.includes( this_val ) ) {
					disable1 = true;
				}
				$( pagination_style + '[value="infinite"]' ).prop( 'disabled', disable1 );
				$( pagination_style + '[value="loadmore"]' ).prop( 'disabled', disable1 );
				if ( disable1 ) {
					$( pagination_style + '[value="regular"]' ).prop( 'checked', true );
					$( pagination_style ).trigger( 'change' );
				}

				$limit.toggle( !disable1 );
			};
