...
if ( !class_exists( 'PT_CV_Hooks_Pro' ) ) {
    ...
    class PT_CV_Hooks_Pro {
        ...
        static function init() {
            ...
            add_filter( PT_CV_PREFIX_ . 'advanced_settings', array( __CLASS__, 'filter_advanced_settings' ) );
            ...
            add_filter( PT_CV_PREFIX_ . 'allowable_tags', array( __CLASS__, 'filter_allowable_tags' ) );
        ...
        static function filter_view_settings( $args ) {
            ...
            if ( strpos( $view_version, 'free' ) !== false ) {
                ...
                if ( version_compare( $view_version, '6.2' ) === -1 ) {
                    $args[ PT_CV_PREFIX . 'extend-scope' ] = '20';
                }
         ...
         * @param type $args
         */
        static function filter_advanced_settings( $args ) {
            ...
            if ( $membership_plugin ) {
                $args[ 'check_access_restriction' ] = sprintf( __( 'Plugin %s: hide restricted posts from unauthorized users', 'content-views-pro' ), "<code>$membership_plugin</code>" );
            }
        ...
        public static function filter_field_thumbnail_dimensions( $args, $fargs ) {
            ...
            if ( PT_CV_Functions::get_global_variable( 'soft_resize' ) === null ) {
                if ( !empty( $exact_size[ 0 ] ) && !empty( $exact_size[ 1 ] ) ) {
                    ...
                    if ( apply_filters( PT_CV_PREFIX_ . 'old_samesize_enable', $soft ) ) {
        ...
        public static function get_inside_image( $post, $dimensions, $content ) {
            ...
            if ( !empty( $matches[ 0 ] ) && apply_filters( PT_CV_PREFIX_ . 'use_simplexml', function_exists( 'simplexml_load_string' ) ) ) {
        ...
        public static function extract_video_url( $content, $post ) {
            ...
            if ( !empty( $media_url ) ) {
                $media_url = str_replace( [ '</p>', '</div>' ], '', $media_url );
        ...
        public static function embed_video( $media_url, $dimensions ) {
            ...
            if ( !empty( $media_url ) ) {
                ...
                if ( $lazyload && !$transient ) {
                    $iframe = str_replace( 'src=', 'data-skip-lazy data-no-lazy class="cvp-videothumb" data-cvpsrc=', $args );
            ...
            $args = apply_filters( PT_CV_PREFIX_ . 'mediathumb_html', $args );
        ...
        public static function filter_btn_more_html( $args, $max_num_pages, $session_id ) {
            ...
            $more_text = !empty( $dargs_pagination[ 'loadmore-text' ] ) ? trim( __cvptranslate( $dargs_pagination[ 'loadmore-text' ], "pagination - load more" ) ) : __( 'More', 'content-views-pro' );
			...
			$args = sprintf(
            '<button class="%s" data-totalpages="%s" data-nextpages="%s" data-sid="%s" data-unid="%s" %s>%s <span class="caret"></span></button>', esc_attr( $more_class ), esc_attr( $max_num_pages ), esc_attr( $next_page ), esc_attr( $session_id ), esc_attr( $cv_unique_id ), $extra_data, esc_html( $more_text )
			);
        ...
        public static function filter_pagination_class( $args ) {
            ...
            if ( isset( $dargs_pagination[ 'type' ] ) && $dargs_pagination[ 'type' ] === 'normal' ) {
                $args .= !empty( $dargs_pagination[ 'output-normal' ] ) ? " {$dargs_pagination[ 'output-normal' ]}" : '';
            } else {
                if ( isset( $dargs_pagination[ 'style' ] ) && $dargs_pagination[ 'style' ] === 'regular' ) {
                    $args .= !empty( $dargs_pagination[ 'output-ajax' ] ) ? " {$dargs_pagination[ 'output-ajax' ]}" : '';
                }
            }
        ...
        public static function filter_field_href_attrs( $custom_attr, $open_in, $oargs = array() ) {
            ...
            // Background image. Skip for scrollable list, to prevent images jump on sliding
        ...
        public static function filter_field_meta_seperator( $args ) {
            ...
            if ( isset( $dargs[ 'field-settings' ][ 'meta-fields' ][ 'hide-slash' ] ) && !empty( $dargs[ 'field-settings' ][ 'meta-fields' ][ 'custom-seperator' ] ) ) {
                $sep  = stripslashes( $dargs[ 'field-settings' ][ 'meta-fields' ][ 'custom-seperator' ] );
                $args = ' ' . wp_kses_post( $sep ) . ' ';
            }
        ...
        public static function filter_field_item_html( $html, $field_, $post ) {
            ...
            switch ( $field_ ) {
                ...
                case 'woorating':
                    if ( $post_type === 'product' && function_exists( 'wc_get_product' ) && function_exists( 'wc_get_rating_html' ) ) {
                        $product = wc_get_product( $post->ID );
                        $rating  = method_exists( $product, 'get_average_rating' ) ? $product->get_average_rating() : '';
                        $html    = $rating ? '<div class="' . PT_CV_PREFIX . 'woorating woocommerce">' . wc_get_rating_html( $rating ) . '</div>' : '';

						// Show CSS for Preview only. In frontend, CSS was loaded by WooCoommerce
						if ( !empty( $html ) && defined( 'PT_CV_DOING_PREVIEW' ) && !isset( $GLOBALS[ 'cvp_preview_custom_css1' ] ) ) {
                            $html .= '<style>/* temporarily added in Preview. no need in frontend */ .woocommerce .star-rating{float:right;overflow:hidden;position:relative;height:1em;line-height:1;font-size:1em;width:5.4em;font-family:star}.woocommerce .star-rating::before{content:"sssss";color:#d3ced2;float:left;top:0;left:0;position:absolute}.woocommerce .star-rating span{overflow:hidden;float:left;top:0;left:0;position:absolute;padding-top:1.5em}.woocommerce .star-rating span::before{content:"SSSSS";top:0;position:absolute;left:0}</style>';
                            $GLOBALS[ 'cvp_preview_custom_css1' ] = 1;
						}
                    }

                    break;

                // Execute Woocommerce hooks
                case 'woohooks':
                    if ( $post_type === 'product' ) {
                        ob_start();
                        ...
                        $extra = ob_get_clean();
                        if ( !empty( $extra ) ) {
                            $html = '<div class="' . PT_CV_PREFIX . 'woohooks">' . $extra . '</div>';
                        }
                    }
				...
				case 'wooprice':
                    if ( $post_type === 'product' && function_exists( 'wc_get_product' ) ) {
						...
						$html = '<div class="' . PT_CV_PREFIX . 'wooprice">' . $price . '</div>';
                    }
                    break;
				case 'wooatc':
                    if ( $post_type === 'product' ) {
						$atc = do_shortcode( sprintf( '[add_to_cart id="%s" style="" show_price="false"]', $post->ID ) );
						$html = '<div class="' . PT_CV_PREFIX . 'wooatc">' . $atc . '</div>';
                    }
                    break;
            }
