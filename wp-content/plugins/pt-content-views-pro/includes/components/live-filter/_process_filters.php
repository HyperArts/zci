...
class CVP_LIVE_FILTER_QUERY {
                ...
                case CVP_LF_PREFIX_TAX:
                    foreach ( $list[ 'selected_filters' ] as $taxonomy => $params ) {
                        ...
                        if ( $lf_or ) {
                            ...
                            $tx_post_query = apply_filters( PT_CV_PREFIX_ . 'lf_tax_query', $tx_post_query );
    ...
    static function query_posts_by_filters( &$args, $query_type, $index, $live_filter_settings = array() ) {
        ...
        // run if this function is run before wp_head in some themes
        if ( !isset( $cvp_lf_params ) ) {
            self::live_filter_url();
        }
    ...
    static function get_operator_of_filter( $filter_type, $array ) {
        switch ( $filter_type ) {
            case CVP_LF_PREFIX_TAX:
                ...
                $operator            = ( isset( $configured_settings[ 'live-filter-type' ] ) && $configured_settings[ 'live-filter-type' ] === 'checkbox' ) ? $configured_settings[ 'live-filter-operator' ] : 'OR';
