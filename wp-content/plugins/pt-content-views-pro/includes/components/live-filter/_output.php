...
class CVP_LIVE_FILTER_OUTPUT {
	...
	public function __construct( $data_type, $input_type, $name, $items, $others, $class = '', $id = '' ) {
		...
		$this->class		 = 'cvp-live-filter ' . apply_filters( PT_CV_PREFIX_ . 'lf_class', $class );
		$this->id			 = $id ? $id : '';
		$this->others		 = $others;
	}
	...
	function _output( $class, $html ) {
		...
		$label = $this->_label();
		return sprintf( '<div class="%s" id="%s" data-name="%s" data-sid="%s" %s>%s</div>', esc_attr( $this->class . $class ), esc_attr( $this->id ), esc_attr( $this->name ), esc_attr( $pt_cv_id ), $extra_data, $label . $html );
	}
	...
	function _sort( $prepend = null ) {
		...
		if ( $lf_arr && $this->data_type === CVP_LF_PREFIX_TAX && is_taxonomy_hierarchical( $this->original_name ) && in_array( $this->input_type, array( 'checkbox', 'radio', 'dropdown' ) ) && !$mansort ) {
			...
			foreach ( $hi_arr as $slug => $indent_text ) {
				...
				/* same as in _text() for text in 'html' */
				$text_in_html = esc_html( $term_info[ 'displaytext' ] );
				if ( in_array( $this->input_type, array( 'checkbox', 'radio' ) ) ) {
					...
					$new_list[]		 = array( 'html' => str_replace( "/>{$text_in_html}<", " $indent_style/>{$text_in_html}<", $term_info[ 'html' ] ) );
				} else {
					$new_list[] = array( 'html' => str_replace( ">{$text_in_html}<", ">{$indent_text}<", $term_info[ 'html' ] ) );
				}
			}
	...
	function _sanitext( $text ) {
		return wp_kses_post( stripslashes( $text ) );
	}
	...
	function radio() {
		$this->items = array( '' => !empty( $this->others[ 'default_text' ] ) ? $this->_sanitext( $this->others[ 'default_text' ] ) : __( 'All', 'content-views-pro' ) ) + $this->items;
	...
	function dropdown() {
		$default		 = !empty( $this->others[ 'default_text' ] ) ? $this->_sanitext( $this->others[ 'default_text' ] ) : (($this->data_type === CVP_LF_SORT) ? __( 'Default', 'content-views-pro' ) : __( 'All', 'content-views-pro' ));
	...
	function range_slider() {
        ...
        $html = sprintf( '<input type="text" name="%s" class="cvp-range-input" data-type="double" data-grid="false" data-min="%s" data-max="%s" data-from="%s" data-to="%s" data-step="%s" data-prefix="%s" data-postfix="%s" data-thousand-separator="%s"/>', esc_attr( $this->name ), esc_attr( $min ), esc_attr( $max ), esc_attr( $from ), esc_attr( $to ), esc_attr( $this->others[ 'step' ] ), esc_attr( $this->_sanitext( $this->others[ 'prefix' ] ) ), esc_attr( $this->_sanitext( $this->others[ 'postfix' ] ) ), esc_attr( $tse ) );
